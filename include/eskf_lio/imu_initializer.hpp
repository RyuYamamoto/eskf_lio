...
#ifndef ESKF_LIO__IMU_INITIALIZER_HPP_
#define ESKF_LIO__IMU_INITIALIZER_HPP_

#include "eskf_lio/lib/math.hpp"
...
public:
  ...
  void add_imu(const sensor_type::Imu & imu_data)
  {
    ...
    if (imu_calibration_time_ < dt) {
      ...
      auto [acc_mean, acc_cov] = math::compute_mean_and_covariance<Eigen::Vector3d>(
        imu_buffer_, [](const sensor_type::Imu & imu) { return imu.linear_acceleration; });
      auto [gyro_mean, gyro_cov] = math::compute_mean_and_covariance<Eigen::Vector3d>(
      ...
      auto [acc_mean_with_gravity, acc_cov_with_gravity] =
        math::compute_mean_and_covariance<Eigen::Vector3d>(
