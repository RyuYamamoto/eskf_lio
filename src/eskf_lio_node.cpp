...
#include "eskf_lio/eskf_lio_node.hpp"

ESKFLioNode::ESKFLioNode(const rclcpp::NodeOptions & options) : Node("eskf_lio", options)
{
  ...
  lio_ = std::make_shared<ESKFLio>(config);
  ...
  points_subscriber_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
    ...
    std::bind(&ESKFLioNode::callback_points, this, std::placeholders::_1), points_sub_opt);
  imu_subscriber_ = this->create_subscription<sensor_msgs::msg::Imu>(
    "imu_raw", rclcpp::SensorDataQoS(),
    std::bind(&ESKFLioNode::callback_imu, this, std::placeholders::_1), imu_sub_opt);
  ...
  timer_ = rclcpp::create_timer(
    ...
    std::bind(&ESKFLioNode::process, this));
}

ESKFLioNode::~ESKFLioNode()
{
}

void ESKFLioNode::process()
{
  ...
  if (lio_->update_local_map(estimated_pose, measurement.lidar_points)) {
    local_map_publisher_thread_ =
      std::thread(&ESKFLioNode::publish_local_map, this, measurement.lidar_points.stamp);
    local_map_publisher_thread_.detach();
  }

  // Publish ROS Message
  publisher_thread_ =
    std::thread(&ESKFLioNode::publish_message, this, measurement, estimated_pose, covariance);
  publisher_thread_.detach();
}

void ESKFLioNode::publish_local_map(const double stamp)
{
...
void ESKFLioNode::publish_message(
  ...
  const Eigen::Matrix<double, 6, 6> & covariance)
{
  ...
  estimated_pose_msg.pose = math::convert_matrix_to_pose(pose.cast<float>());
...
double ESKFLioNode::get_lidar_scan_duration(const sensor_msgs::msg::PointCloud2::SharedPtr & msg)
{
  double scan_duration = 0.0;

  auto find_timestamp_field = [&msg]() -> std::string_view {
    constexpr std::array<std::string_view, 4> fields = {"t", "time", "timestamp", "time_stamp"};
    auto it = std::find_if(msg->fields.begin(), msg->fields.end(), [&](const auto & field) {
      return std::find(fields.begin(), fields.end(), field.name) != fields.end();
    });
...
void ESKFLioNode::callback_points(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
  PointCloudPtr cloud(new PointCloud);
  pcl::fromROSMsg(*msg, *cloud);

  const double scan_duration = get_lidar_scan_duration(msg);
  ...
  pcl::transformPointCloud(
    *cloud, *base_to_sensor_cloud, math::convert_transform_to_matrix(base_to_sensor));
...
void ESKFLioNode::callback_imu(const sensor_msgs::msg::Imu::SharedPtr msg)
{
  ...
  Eigen::Transform<double, 3, Eigen::Affine> transform = math::get_eigen_transform(base_to_imu);
...
bool ESKFLioNode::get_transform(
...
RCLCPP_COMPONENTS_REGISTER_NODE(ESKFLioNode)
