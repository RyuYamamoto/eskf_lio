...
void ESKF::predict(const sensor_type::Imu imu_measurement)
{
  ...
  Fx_.block<3, 3>(3, 6) = -R * math::skew_symmetric_matrix(acc) * dt;
...
Eigen::Matrix4d ESKF::update(sensor_type::Pose & pose_measurement)
{
  ...
  residual.tail<3>() = math::convert_matrix_to_euler(
  ...
  // reset ESKF
  G_.block<3, 3>(6, 6) =
    Eigen::Matrix3d::Identity() - 0.5 * math::skew_symmetric_matrix(error_.block<3, 1>(6, 0));
  state_.P = G_ * state_.P * G_.transpose();
  error_.setZero();

  return math::get_matrix(state_.position, state_.quaternion);
}
